...
def generate_market_sections(
    *,
    ...
    prompt = (
        ...
        "Executive Summary는 후속 단계에서 전체 섹션을 모두 참조해 별도로 작성되므로, 현재 작성하는 각 절은 정보 손실 없이 최대한 구체적으로 남긴다. "
...
def generate_skhynix_sections(
    *,
    ...
    prompt = (
        ...
        "Executive Summary는 후속 단계에서 전체 섹션을 모두 참조해 별도로 작성되므로, 현재 작성하는 절은 사례와 근거를 생략하지 않는다. "
        "다른 LLM이 남긴 요약본도 정보 손실 없이 이어받아 반영한다. "
...
def generate_technique_sections(
    *,
    ...
    prompt = (
        ...
        "Executive Summary는 후속 단계에서 전체 섹션을 모두 참조해 별도로 작성되므로, 현재 작성하는 절은 기술 구조와 효과를 압축하지 않는다. "
        "다른 LLM이 남긴 요약본도 정보 손실 없이 이어받아 반영한다. "
        f"질문: {user_query}. 근거 요약: {evidence_digest} 다른 LLM 요약본: {peer_context}"
    )
    return _json_completion(client=client, model=model, prompt=prompt)
...
def evaluate_report_sections(*, client: OpenAI, model: str, user_query: str, section_summaries: dict[str, str]) -> dict[str, Any]:
    prompt = (
        ...
        "평가 기준은 평서문 사용, 정량 수치 포함, 정량 수치에 지표 이름 동반 여부, 관련 뉴스 포함 여부, 정보 손실 없는 구체적 설명, 사례의 4요소 형식 유지, 미래 방향성의 근거 제시, 추론 근거 불릿 3개 이상, Limitation 불릿 3개 이상, Executive Summary가 15줄을 초과하며 전체 섹션을 반영하는지 여부, 분석 배경의 미래전략 집중 이유와 결론 정리, 경쟁사별 전략/미래 계획/TRL 비교, 핵심 기술 현황의 3줄 요약과 장단점 불릿, HTML 태그 미사용, 참고문헌이 번호형 각주만 포함하는지 여부, TRL 4~6 적합성이다. "
